...
object DXVKHelper {
    const val DEFAULT_CONFIG: String = "version=" + DefaultVersion.DXVK + ",framerate=0,maxDeviceMemory=0"

    fun parseConfig(config: Any?): KeyValueSet {
        val data = if (config != null && !config.toString().isEmpty()) config.toString() else DEFAULT_CONFIG
        return KeyValueSet(data)
    }

    fun setEnvVars(context: Context?, config: KeyValueSet, envVars: EnvVars) {
        envVars.put("DXVK_STATE_CACHE_PATH", ImageFs.CACHE_PATH)
        envVars.put("DXVK_LOG_LEVEL", "none")

        val rootDir: File = ImageFs.find(context!!).rootDir
        val dxvkConfigFile = File(rootDir, ImageFs.CONFIG_PATH + "/dxvk.conf")

        var content = ""
        val maxDeviceMemory = config.get("maxDeviceMemory")
        if (maxDeviceMemory.isNotEmpty() && maxDeviceMemory != "0") {
            content += "dxgi.maxDeviceMemory = $maxDeviceMemory\n"
            content += "dxgi.maxSharedMemory = $maxDeviceMemory\n"
        }

        val framerate = config.get("framerate")
        if (framerate.isNotEmpty() && framerate != "0") {
            content += "dxgi.maxFrameRate = $framerate\n"
            content += "d3d9.maxFrameRate = $framerate\n"
        }

        FileUtils.delete(dxvkConfigFile)
        if (content.isNotEmpty() && FileUtils.writeString(dxvkConfigFile, content)) {
