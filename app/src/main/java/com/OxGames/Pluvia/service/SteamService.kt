...
class SteamService : Service(), IChallengeUrlChanged {
    ...
    // Files
    private val serverListPath by lazy { cacheDir.toPath().resolve("server_list.bin") }
    ...
    internal lateinit var steamClient: SteamClient
    ...
    private lateinit var steamFamilyGroups: FamilyGroups
    private lateinit var steamFriends: SteamFriends
    private lateinit var steamUser: SteamUser
    private lateinit var unifiedFriends: SteamUnifiedFriends
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        if (!isRunning) {
            ...
            val configuration = SteamConfiguration.create {
                it.withProtocolTypes(ProtocolTypes.WEB_SOCKET)
            ...
            steamUser = requireNotNull(steamClient.getHandler(SteamUser::class.java))
            steamApps = requireNotNull(steamClient.getHandler(SteamApps::class.java))
            steamFriends = requireNotNull(steamClient.getHandler(SteamFriends::class.java))
            steamCloud = requireNotNull(steamClient.getHandler(SteamCloud::class.java))

            steamFamilyGroups = requireNotNull(steamClient.getHandler<SteamUnifiedMessages>()).createService<FamilyGroups>()
            ...
            with(callbackSubscriptions) {
                with(callbackManager) {
                    ...
                    add(subscribe<PersonaStateCallback> { ::onPersonaStateReceived })
                    add(subscribe<LicenseListCallback> { ::onLicenseList })
                    ...
                    add(subscribe<AliasHistoryCallback> { ::onAliasHistory })
                    add(subscribe<PICSChangesCallback> { ::onPicsChanges })
                    add(subscribe<PICSProductInfoCallback> { ::onPicsProduct })
                }
            }
            ...
            scope.launch {
                while (isRunning) {
                    callbackManager.runWaitCallbackAsync()
                }
            }
