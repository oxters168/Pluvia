...
import com.OxGames.Pluvia.ui.screen.login.components.TwoFactorTextField
...
fun TwoFactorAuthScreenContent(
    ...
    Column(
        ...
        when (userLoginState.loginResult) {
            LoginResult.DeviceConfirm -> LinearProgressIndicator(modifier = Modifier.fillMaxWidth())
            LoginResult.EmailAuth,
            LoginResult.DeviceAuth,
            -> {
                TwoFactorTextField(
            ...
            else -> throw IllegalArgumentException("Invalid 2FA Screen ${userLoginState.loginResult}")
        }
    }
}
